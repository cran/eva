...
The previous chunk of code generates data from a correlated multivariate normal random variable and the following chunk transforms the data onto the uniform scale. We can see the transformed (uniform) random variables are still correlated.
...
Next, we transform the uniform data into GEV margins with site specific location means [8, 10, 12, 9] and a shared location trend of 0.1. The design matrix labels each of the four sites as A, B, C, or D. The estimated means can be constructed from the output. Site A is chosen as the reference level, so its location parameter is estimated by the location intercept -- 8.337. The other site specific location parameter estimates can be obtained by adding the intercept and the corresponding site value. For example, site C has a location estimate of 8.337 + 3.844 = 12.181.
